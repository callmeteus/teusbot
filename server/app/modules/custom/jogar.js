	...
	content: 										function(processor) {
		...
		if (cmd === undefined) {
			...
			if (this.module.playListOpen === true) {
				...
				if (this.module.playListAll.findIndex((n) => n.toLowerCase() === nickname.toLowerCase()) > -1) {
					return processor.sendMessage(`❌ ${processor.sender.nickname} você já jogou durante a stream atual.`);
				} else
				// Checa se a pessoa já jogou durante a stream
				if (this.module.playList.findIndex((n) => n.toLowerCase() === nickname.toLowerCase()) > -1) {
				...
				if (this.module.playListSpecial.findIndex((n) => n.toLowerCase() === nickname.toLowerCase()) > -1) {
