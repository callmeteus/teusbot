module.exports 									= function() {
	...
	this.on("chat.command", function(processor) {
			...
			if (processor.sender.isMod) {
				...
				switch(processor.arguments[0]) {
					...
					case "start":
						...
						return processor.sendMessage(this.getLangMessage("RAFFLE_START"));
					break;
					...
					case "end":
						if (entries.length) {
							...
							return processor.sendMessage(this.getLangMessage("RAFFLE_WINNER", {
								raffle: 		{
									entries: 	entriesCount,
		...
		// Check if nickname is already in the entries
		if (entriesOpen && entries.indexOf(processor.sender.id) === -1) {
